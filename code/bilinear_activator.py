...
from bilinear_model import BilinearModule, LayeredBilinearModule
from dataset import BilinearDataset
from datset_sampler import ImbalancedDatasetSampler
from parameters import RefaelDatasetParams, BilinearDatasetParams, BilinearModuleParams, BilinearActivatorParams, \
    LayeredBilinearModuleParams, AidsDatasetTestParams, AidsDatasetDevParams, AidsDatasetTrainParams, \
    MutagenDatasetTrainParams, MutagenDatasetDevParams, MutagenDatasetTestParams
...
class BilinearActivator:
    def __init__(self, model: BilinearModule, params: BilinearActivatorParams, train_data: BilinearDataset,
    ...
    def plot_line(self, job=LOSS_PLOT):
        p = figure(plot_width=600, plot_height=250, title="Refael - Dataset - " + job,
    ...
    def train(self, show_plot=True):
        ...
        for epoch_num in range(self._epochs):
            ...
            for batch_index, (A, D, x0, l) in enumerate(self._balanced_train_loader):
                ...
                output = self._model(A, D, x0)                  # calc output of current model on the current batch
    ...
    def _validate(self, data_loader, job=""):
        ...
        for batch_index, (A, D, x0, l) in enumerate(data_loader):
            # print progress
            self._print_progress(batch_index, len_data, job=VALIDATE_JOB)
            output = self._model(A, D, x0)
        ...
        self._update_auc(pred_labels, true_labels, job=job)
        return loss
...
if __name__ == '__main__':
    ...
    # aids_train_ds = BilinearDataset(MutagenDatasetTrainParams())
    # aids_dev_ds = BilinearDataset(MutagenDatasetDevParams())
    # aids_test_ds = BilinearDataset(MutagenDatasetTestParams())
    activator = BilinearActivator(LayeredBilinearModule(LayeredBilinearModuleParams(ftr_len=aids_train_ds.len_features)),
                                  BilinearActivatorParams(), aids_dev_ds, dev_data=aids_dev_ds, test_data=aids_dev_ds)
